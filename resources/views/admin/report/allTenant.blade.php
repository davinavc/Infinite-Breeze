...
<head>
    ...
    <title>Report All Tenant</title>
    <link rel="stylesheet" href="{{ public_path('css/report.css') }}">
</head>
<body>
    <h1>Report All Tenant</h1>
    ...
    @foreach($tenants as $tenant)
        <table class="table-data-event">
            <tr>
                <!-- Gambar -->
                <td class="td-image">
                    <img src="{{ public_path('storage/' . $tenant->logo) }}" alt="Poster" style="max-width: 100%; height: 200px;">
                </td>
                ...
                <td style="vertical-align: top; padding-left: 20px;text-align:left;">
                    ...
                    <p><strong>Phone Number:</strong> {{ $tenant->user->no_telp }}</p>
                    <p><strong>Account Created:</strong> {{ $tenant->created_at->format('d-m-Y') }}</p>
                    <p><strong>Joined Event:</strong> {{ $tenant->joined_event }}</p>
                </td>
            </tr>
        </table>
        <h4>Joined event</h4>
                ...
                <tbody>
                    ...
                    @foreach($tenant->registrations as $reg)
                        @php
                            $trx = $tenant->transaksi->where('event_id', $reg->event_id)->first();
                        @endphp
                        <tr>
                            <td>{{ $no++ }}</td>
